...
class Answer(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True, nullable=False)  # 답변 번호
    ...
    persuasive = Column(Integer, nullable=True)  # 설득 
    depth = Column(Integer, nullable=True)  # 깊이 

    # 평가 근거
    creativity_evid = Column(Text, nullable=True)  
    logic_evid = Column(Text, nullable=True)  
    thinking_evid = Column(Text, nullable=True) 
    persuasive_evid = Column(Text, nullable=True) 
    depth_evid = Column(Text, nullable=True)  
